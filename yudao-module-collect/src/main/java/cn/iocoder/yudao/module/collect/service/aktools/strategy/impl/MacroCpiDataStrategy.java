...
import cn.iocoder.yudao.module.collect.service.aktools.converter.AkToolsMacroCpiDataConverter;
...
public class MacroCpiDataStrategy extends AbstractAkToolsDataStrategy<AkToolsMacroCpiDO> {

    private final AkToolsMacroCpiDataConverter macroCpiDataConverter;

    @Override
    public AkApiEnum getApiEnum() {
        return AkApiEnum.MACRO_CPI;
    }
    ...
    @Override
    public AkToolsMacroCpiDO convertData(Object rawData, ConversionContext context) {
        if (!(rawData instanceof Map)) {
            throw new IllegalArgumentException("原始数据格式错误，期望Map类型");
        }
        
        Map<String, Object> dataMap = (Map<String, Object>) rawData;
        return macroCpiDataConverter.convert(dataMap, context.getTradeDate());
    }
